...
def QuantumPasswordTime(password: str, rate: float):
    length = len(password)
    ...
    maxstates = 2 ** 433
    if maxstates < N:
        return "Infinite time! The password is longer than any quantum computer can handle."
    else:
        seconds = ((math.pi / 4.0) * math.sqrt(N)) / rate
        return _siprefix(seconds)